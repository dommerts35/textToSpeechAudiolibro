...
class AudioManager:
    ...
    def _convert_chunk(self, text: str, output_path: str, language: str) -> bool:
        ...
        try:
            # Asegurar que el texto no exceda los límites
        ...
        try:
            ...
            for i, chunk in enumerate(chunks):
                ...
                if self._convert_chunk(chunk, chunk_path, language):
                    ...
                    # Continuar con los demás chunks en lugar de fallar completamente
                    continue
            ...
            # En una versión avanzada, aquí se combinarían con ffmpeg
            # Por ahora, usamos el primer chunk exitoso y renombramos
            if audio_files:
                os.rename(audio_files[0], output_path)
                self.logger.info(f"Texto largo convertido (usando primer chunk): {output_path}")

                # Limpiar chunks temporales restantes
